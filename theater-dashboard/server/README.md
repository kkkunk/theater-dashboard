...
后端为 Express + SQLite，默认监听 `http://localhost:3001`。当前数据库是可复现的演示数据，报告基准日为 2026-08-07。
...
| GET | `/api/dashboard/audience` | 年龄、地域、圈层及新老客 |
...
- 复购率：周期内复购订单数 / 总订单数。
- 新媒体声量：小红书笔记 + 公众号次数 + 外部评论 + 抖音点赞 / 100；该值是演示用归一化指数，不等同于真实曝光量。
