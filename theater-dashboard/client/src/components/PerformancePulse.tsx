...
export function PerformancePulse({ summary, mediaSummary, scopeDays, onScopeChange }: { summary: Summary; mediaSummary: Summary; scopeDays: 180 | 365; onScopeChange: (days: 180 | 365) => void }) {
  ...
  const metrics = [
    ...
    { label: '上座率完成度', value: summary.metrics.occupancyRate.available ? `${summary.metrics.occupancyRate.value.toFixed(1)}%` : '暂无数据', note: summary.metrics.occupancyRate.available ? `有容量项目累计出票 ${formatNumber(summary.metrics.totalIssuedTickets.value)} 张` : '缺少可出票数', formula: '有可出票数据项目的累计总出票数 ÷ 可出票数 × 100%；总出票数包含工作票。' },
  ];
