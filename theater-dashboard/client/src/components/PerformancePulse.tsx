...
export function PerformancePulse({ summary, mediaSummary, scopeDays, onScopeChange }: { summary: Summary; mediaSummary: Summary; scopeDays: 180 | 365; onScopeChange: (days: 180 | 365) => void }) {
  ...
  const metrics = [
    { label: '售票完成率', value: summary.metrics.salesCompletionRate.available ? `${summary.metrics.salesCompletionRate.value.toFixed(1)}%` : '暂无目标', note: `全部项目累计至 ${cutoffLabel} · 工作票 ${formatNumber(summary.metrics.workTickets.value)} 张已排除` },
    { label: '票房完成度', value: summary.metrics.boxOfficeCompletionRate.available ? `${summary.metrics.boxOfficeCompletionRate.value.toFixed(1)}%` : '暂无预估', note: `全部项目累计至 ${cutoffLabel}` },
    { label: '上座率完成度', value: summary.metrics.occupancyRate.available ? `${summary.metrics.occupancyRate.value.toFixed(1)}%` : '暂无数据', note: summary.metrics.occupancyRate.available ? `有容量项目累计出票 ${formatNumber(summary.metrics.totalIssuedTickets.value)} 张` : '缺少可出票数' },
  ];
  return <motion.section className="pulse-panel pulse-panel-v3" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55 }}>
    ...
    <div className="pulse-media-core">
      ...
      <div><span>平台净增粉丝</span><strong>{mediaSummary.metrics.mediaFollowerGrowth.available ? `${mediaSummary.metrics.mediaFollowerGrowth.value > 0 ? '+' : ''}${formatNumber(mediaSummary.metrics.mediaFollowerGrowth.value)}` : '暂无数据'}</strong><small>期末粉丝数 − 期初粉丝数</small></div>
      <div><span>平台互动量</span><strong>{mediaSummary.metrics.mediaInteractions.available ? formatNumber(mediaSummary.metrics.mediaInteractions.value) : '暂无数据'}</strong><small>点赞 + 评论 + 转发</small></div>
      <div><span>总宣传量</span><strong>{formatNumber(mediaSummary.metrics.mediaVolume.value)}</strong><small>实际发布内容数</small></div>
    </div>
  </motion.section>;
}
