...
export function PerformancePulse({ summary }: { summary: Summary }) {
  ...
  const metrics = [
    { label: '售票完成率', value: summary.metrics.salesCompletionRate.available ? `${summary.metrics.salesCompletionRate.value.toFixed(1)}%` : '暂无目标', note: `截至 ${cutoffLabel} 累计 · 工作票 ${formatNumber(summary.metrics.workTickets.value)} 张已排除` },
    { label: '票房完成度', value: summary.metrics.boxOfficeCompletionRate.available ? `${summary.metrics.boxOfficeCompletionRate.value.toFixed(1)}%` : '暂无预估', note: `截至 ${cutoffLabel} 累计票房 / 预计票房` },
    { label: '上座率完成度', value: summary.metrics.occupancyRate.available ? `${summary.metrics.occupancyRate.value.toFixed(1)}%` : '暂无数据', note: summary.metrics.occupancyRate.available ? `截至 ${cutoffLabel} 累计出票 ${formatNumber(summary.metrics.totalIssuedTickets.value)} 张` : '缺少可出票数' },
  ];
