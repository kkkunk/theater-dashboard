...
export function PerformancePulse({ summary }: { summary: Summary }) {
  const metrics = [
    ...
    { label: '付费上座率', value: `${summary.metrics.occupancyRate.value.toFixed(1)}%`, note: '赠票不计入分子' },
  ];
  return <motion.section className="pulse-panel pulse-panel-v3" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55 }}>
    ...
    <div className="pulse-media-core">
      ...
      <div><span>平台净增粉丝</span><strong>+{formatNumber(summary.metrics.mediaFollowerGrowth.value)}</strong><small>期末粉丝数 − 期初粉丝数</small></div>
      <div><span>平台互动量</span><strong>{formatNumber(summary.metrics.mediaInteractions.value)}</strong><small>点赞 + 评论 + 转发</small></div>
      <div><span>总宣传量</span><strong>{formatNumber(summary.metrics.mediaVolume.value)}</strong><small>五个平台汇总</small></div>
    </div>
  </motion.section>;
}
